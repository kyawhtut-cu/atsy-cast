...
let forceConvert = (str) => {
  const _isZawgyi = isZawgyi(str)
  if(_isZawgyi) return zg2uni(str)
  return str
}

let replaceWithRule = (rule, str) => {
...
let isZawgyi = (str) => {
  const regx = new RegExp("\u1039[\u1000-\u1021]\u103A|[\u105A\u1060-\u1097]" +
...
let testIsZawgyi = () => {
  Logger.log(`Zawgyi Result => ` + isZawgyi(`သီဟိုဠ္မွ ဉာဏ္ႀကီးရွင္သည္ အာယုဝၯနေဆးၫႊန္းစာကို ဇလြန္ေဈးေဘး ဗာဒံပင္ထက္ အဓိ႒ာန္လ်က္ ဂဃနဏဖတ္ခဲ့သည္။`))
  Logger.log(`Unicode Result => ` + isZawgyi(`သီဟိုဠ်မှ ဉာဏ်ကြီးရှင်သည် အာယုဝဍ္ဎနဆေးညွှန်းစာကို ဇလွန်ဈေးဘေး ဗာဒံပင်ထက် အဓိဋ္ဌာန်လျက် ဂဃနဏဖတ်ခဲ့သည်။`))
}
